...
public class Door : MonoBehaviour
{
    ...
    {
        if(collision.CompareTag("Player") && Input.GetKeyDown(KeyCode.N))
        {
            ...
            else if (transform.name == "Door7")
            {
                SceneManager.UnloadSceneAsync("Palm Island");
                SceneManager.LoadScene("Cave 4", LoadSceneMode.Additive);
                Events.ChangeScene("Cave 4");
                GameObject.Find("Player").transform.position = new Vector3(25.8f, 37.5f, 0);
            }

            else if (transform.name == "Door8")
            {
                SceneManager.UnloadSceneAsync("Cave 4");
                SceneManager.LoadScene("Palm Island", LoadSceneMode.Additive);
                Events.ChangeScene("Palm Island");
                GameObject.Find("Player").transform.position = new Vector3(-3.8f, 15f, 0);
            }
        }
    }
}
